...
export class OnlyNumbers {
    ...
    decimalWithFourDecimals: RegExp = new RegExp(/^\d*\.?\d{0,4}$/g);
    ...
    @HostListener('keydown', ['$event']) onKeyDown(e: KeyboardEvent) {
        ...
        const allowedKeys = [
            'Backspace', 'ArrowLeft', 'ArrowRight', 'Escape', 'Tab', 'End', 'Home', 'Del', 'Delete'
        ];
        ...
        // Permitir numero con 4 decimales
        const position = this.hostElement.nativeElement.selectionStart;
        const next: string = [originalValue.slice(0, position), e.key == 'Decimal' ? '.' : e.key, originalValue.slice(position)].join('');
        if (next && !String(next).match(this.decimalWithFourDecimals)) {
            e.preventDefault();
        }
        //
    ...
    validateValue(value: string): void {
        ...
        if (valid)
        {
            this.hostElement.nativeElement['value'] = this.currencyPipe.transform(value);
            //this.hostElement.nativeElement['value'] = '€' + value;
        } else {
            this.hostElement.nativeElement['value'] = 0;
        }
